...
import static org.smartregister.chw.pmtct.util.Constants.STEP_ONE;
import static org.smartregister.chw.pmtct.util.Constants.STEP_TWO;

public class PmtctJsonFormUtils extends org.smartregister.util.JsonFormUtils {
    ...
    protected static FormTag formTag(AllSharedPreferences allSharedPreferences) {
        ...
        formTag.appVersion = PmtctLibrary.getInstance().getApplicationVersion();
        formTag.databaseVersion = PmtctLibrary.getInstance().getDatabaseVersion();
        return formTag;
    }
    ...
    public static void tagEvent(AllSharedPreferences allSharedPreferences, Event event) {
        ...
        event.setClientApplicationVersion(PmtctLibrary.getInstance().getApplicationVersion());
        event.setClientDatabaseVersion(PmtctLibrary.getInstance().getDatabaseVersion());
    }
    ...
    public static JSONObject getFormAsJson(String formName) throws Exception {
        return FormUtils.getInstance(PmtctLibrary.getInstance().context().applicationContext()).getFormJson(formName);
    }
